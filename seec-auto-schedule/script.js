...
document.getElementById('stationForm').addEventListener('submit', function(e) {
    ...
    const station = {
        ...
        excludeFromSchedule: false,  // 新增不列入排班屬性
        lineId: lineId
    };
                        ...
                        return station ? 
                            `<div class="station-item">
                                ...
                                <div class="station-controls">
                                    <label class="checkbox-label exclude-toggle">
                                        <input type="checkbox" 
                                               ${station.excludeFromSchedule ? 'checked' : ''}
                                               onchange="toggleExcludeStation('${station.id}', this.checked)">
                                        <span>不參與排班</span>
                                    </label>
    ...
    modal.innerHTML = `
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                    <label class="checkbox-label">
                        <input type="checkbox" id="editExcludeFromSchedule" ${station.excludeFromSchedule ? 'checked' : ''}>
                        <span>不列入排班</span>
                    </label>
                </div>
            </div>
...
function saveStationEdit(stationId) {
    ...
    const newExcludeFromSchedule = document.getElementById('editExcludeFromSchedule').checked;
    ...
    station.excludeFromSchedule = newExcludeFromSchedule;
...
function executeTask() {
    if (confirm('確定要執行排程嗎？')) {
        // 過濾掉不列入排班的工作站
        const schedulableStations = stations.filter(s => !s.excludeFromSchedule);
        
        const data = {
            personnel: personnel,
            productionLines: productionLines,
            stations: schedulableStations,
            timestamp: new Date().toISOString()
        };

        document.body.classList.add('loading');
        
        // 使用您的 Python Anywhere 或其他後端服務地址
        fetch('https://your-pythonanywhere-username.pythonanywhere.com/api/schedule', {
            method: 'POST',
            headers: {
...
// 添加切換工作站排班狀態的函數
function toggleExcludeStation(stationId, excluded) {
    const station = stations.find(s => s.id === stationId);
    if (station) {
        station.excludeFromSchedule = excluded;
        autoSaveSettings();
    }
}
